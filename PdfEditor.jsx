...
const PdfEditor = () => {
  ...
  const [pdfDimensions, setPdfDimensions] = useState({ width: 800, height: 1130 });
  ...
  const executeTool = async () => {
    ...
    try {
      ...
      for (const el of elements) {
        ...
        // Since the editor now matches the exact native dimensions of the PDF, 
        // scaling is exactly 1:1 and no padding offsets are needed.
        const scaleX = 1; 
        const uniformScale = 1; 

        const adjustedX = el.x !== undefined ? el.x : 50;
        const adjustedY = el.y !== undefined ? el.y : 50;

        if (el.type === 'text') {
          const rgbColor = hexToRgb(el.color);
          targetPage.drawText(el.text || 'Sample Text', {
            x: adjustedX * uniformScale,
            y: pdfHeight - (adjustedY * uniformScale) - (el.size * uniformScale * 0.8),
            size: (el.size || 16) * uniformScale,
          ...
          if (pdfImage) {
            targetPage.drawImage(pdfImage, { 
              x: adjustedX * uniformScale, 
              y: pdfHeight - (adjustedY * uniformScale) - ((el.imgHeight || 150) * uniformScale), 
              width: (el.imgWidth || 150) * uniformScale, 
              height: (el.imgHeight || 150) * uniformScale 
            });
          }
        }
          ...
          targetPage.drawRectangle({ 
            x: adjustedX * uniformScale, 
            y: pdfHeight - (adjustedY * uniformScale) - ((el.height || 50) * uniformScale), 
            width: (el.width || 100) * uniformScale, 
            height: (el.height || 50) * uniformScale, 
            color: rgb(1, 1, 1) 
          });
        }
      }
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: '#e0e0e0' /* Darker bg for canvas area to match Acrobat */ }}>
          ...
          >
            <div 
              ...
              style={{ 
                position: 'relative', 
                width: `${pdfDimensions.width * (zoom / 100)}px`, 
                height: `${pdfDimensions.height * (zoom / 100)}px`, 
                background: '#fff', 
            ...
            >
              ...
              <div 
                ...
                style={{ 
                  position: 'absolute', top: 0, left: 0, width: `${pdfDimensions.width}px`, height: `${pdfDimensions.height}px`,
